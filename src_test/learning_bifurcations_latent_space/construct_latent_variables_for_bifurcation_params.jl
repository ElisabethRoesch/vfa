...
for temp_ind_alpha in ind_alphas
    for temp_ind_init in ind_inits
        scatter!(t, xes[counter_a[1], counter_i[1]][1,:], label = "")
        counter_i[1]+=1
    end
    counter_i[1]=1
    counter_a[1]+=1
...
for temp_ind_alpha in ind_alphas
    temp_ind_init = 3
    scatter!(t, xes[counter_a[1], temp_ind_init][1,:], label = "")
    counter_a[1]+=1
end
...
for temp_ind_alpha in ind_alphas
    temp_ind_init = 3
    plot!(t, xes[counter_a[1], temp_ind_init][1,:], xes[counter_a[1], temp_ind_init][2,:], label = "")
    counter_a[1]+=1
end
